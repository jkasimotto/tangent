...
function buildCommandDeps(input: WiringInput, reads: RuntimeReads, publish: ReturnType<typeof buildPublishDeps>, surfaces: SurfaceInput, openSurface: (id: SurfaceId, opener?: HTMLElement | null) => void, closeSurface: (id: SurfaceId) => void): CommandDeps {
  const { core, stores } = input;
  return {
    controller: core.controller,
...
function buildEscape(input: WiringInput, surfaces: SurfaceInput, commands: CommandDeps): () => { readonly kind: string } {

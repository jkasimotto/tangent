...
Status: accepted. Supersedes ADR-0018. The `tangent goal handover` clauses are amended by ADR-0040: a worker runs `tangent send brain`, and the brain, not the server, starts the next step. The review-closure clause is superseded by ADR-0041: no report closes a Goal, the brain runs `tangent goal done`.
...
- Effort becomes a third axis of the harness registry (`~/.tangent/trees/harnesses.md`, `effortSets`); the composed command is harness + model + effort. A model can select a narrower effort set than its harness, so the picker only offers levels that the chosen model supports (for example, Codex Sol can offer Ultra while Luna stops at Max).

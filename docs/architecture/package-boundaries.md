...
Vertical apps:
- @tangent/agent-shell owns the vault CLI (`tangent area`, `tangent goal`, `tangent vault commit`), the agent messaging CLI (`tangent agent list|send`), the pipeline CLI (`tangent goal start`, `tangent goal append`), the brain CLI (`tangent brain status|stop|request`, `tangent area create`), the server CLI (`tangent shell rebuild`), and `tangent study`, lazily loaded from `@tangent/agent-shell/cli`. Every command but `vault commit` and `study` is a thin HTTP client to the Agent Shell server. `vault commit` writes the vault history. `study` starts one local interactive session.
- The Agent Shell app in `packages/agent-shell/app/` has one stable gateway and one supervised controller (ADR-0032). They share one runtime identity. The session ownership capability creates, attaches, and terminates only tmux sessions with that identity (ADR-0036). The gateway owns the public listener, terminal transport, static assets, and SSE. The controller owns exact-Area logical brains, the root harness registry, explicit inherited per-Area `harnesses.md` contracts, launch memory, activation and memory projections, durable Area inboxes, Requests, presented-document attention, Goal queues, worker handover notice outboxes, Operation event outboxes, and detached audit exports (ADR-0054). Capability route tables own controller HTTP shapes. Browser factories receive feature-owned ports. `public/shell.js` is the browser composition root.

...
- The Agent Shell app in `packages/agent-shell/app/` has one stable gateway and one supervised controller (ADR-0032). They share one runtime identity. The session ownership capability creates, attaches, and terminates only tmux sessions with that identity (ADR-0036). The gateway owns the public listener, terminal transport, static assets, and SSE. The controller owns exact-Area Brains, Goal intent projection, `job.v1` history, Agent Attempts, the harness registry and Area policies, durable inboxes and receipt recovery, Requests, presentations, structured lifecycle events, invariant Problems, and detached audit exports (ADR-0054, ADR-0055). No other vertical package imports these runtime record types.

...
export function clampRollupConversation(conversation: NormalizedConversation, config: RollupConfig): NormalizedConversation {
...
export function compactRollupCaveats(caveats: string[], limit: number): string[] {
  ...
  if (entries.length > limit) {
    selected.push(`${entries.length - limit} additional caveat types were omitted from the roll-up input.`);
  }
  return selected;
}
...
function compactRollupConversation(conversation: NormalizedConversation, config: RollupConfig, options: RollupClampOptions): NormalizedConversation {
  return {
    ...conversation,
    ...
    caveats: compactRollupCaveats([
      ...
      "Conversation report was truncated for roll-up input."
    ], 8)
  };
}

function compactRollupMessage(message: NormalizedConversationMessage, config: RollupConfig, options: RollupClampOptions): NormalizedConversationMessage {
...
function compactRollupTools(tools: NormalizedToolCall[], config: RollupConfig, options: RollupClampOptions): NormalizedToolCall[] {
...
function hardClampRollupConversation(conversation: NormalizedConversation, config: RollupConfig): NormalizedConversation {
  ...
  const base = {
    ...
    caveats: compactRollupCaveats([
      ...conversation.caveats,
      "Conversation report was heavily truncated for roll-up input; user messages were preserved first."
    ], 8)
  };
...
function normalizeRollupCaveat(caveat: string): string {
  ...
  if (/^Conversation report was heavily truncated/.test(caveat)) {
    return "Conversation report was heavily truncated for roll-up input; user messages were preserved first.";
  }
  if (/^Conversation report was truncated/.test(caveat)) {
    return "Conversation report was truncated for roll-up input.";
  }

...
Install rule: Tangent stays in one git monorepo, but each vertical app package must be publishable and usable on its own. Standalone installs can depend on platform packages. Rollup and Eval can also depend on Usage data packages. They must not pull unrelated vertical apps or Usage UI packages. Package manifests use normal semver `@tangent/*` dependency ranges. Publishable manifests cannot use workspace-local dependency specifications.
...
One vault Document, `map-kinds.md`, decides what each kind of thing looks like on a Map and what one click does with it. Drawings live in `map-icons/`. A Block whose kind has an icon renders as a figure: one large drawing with a caption beside it. The icon is a locked ephemeral projection element, like the success rail, so it never enters a source shard, a fingerprint, or a hit test. A kind with no usable icon keeps its card, and a problem in the definition never hides a Block.
...
Work infers no ask. A Question exists only when a brain writes a Request, and it stays with that brain: a quiet count on the Area header, and a deliberate review behind it. The controller publishes one bounded `agent-shell-work.v3` candidate after authoritative source reads. The gateway validates and atomically stores it. `GET /api/work` serves only the immutable gateway buffer. A refresh failure keeps the last complete revision visible as `Last known` (ADR-0056).
...
Each accepted worker Area note adds a receipt to that Goal queue before it writes one organizer inbox notice. A pending receipt is a durable notice outbox. A stable source ID makes retry and recovery idempotent.
Workers send ordinary notes, finish with `--done`, or name a real dependency with `--blocked`. Worker sends do not create a synchronous question lifecycle. Brain Requests to Julian and harness decision dialogs remain separate mechanisms.
...
Exact-Area messages enter the durable Area inbox before live delivery.
...
The package is lazily loaded from `@tangent/agent-shell/cli` for the root `area`, `brain`, `goal`, and `vault` commands (ADR-0020). Area and Goal commands are thin HTTP clients to the stable Agent Shell gateway on port 4321. The gateway proxies them to the controller. `vault commit` is the one exception. It commits directly to `~/.tangent/trees` with provenance trailers.

Agent Shell is the daily work product. The app in `packages/agent-shell/app/` connects Areas, Goals, Documents, tmux sessions, and attention signals. `gateway.mjs` owns the public listener, static assets, SSE, terminal transport, and the atomic Work store. It supervises the replaceable `server.mjs` controller over an ephemeral loopback port and IPC heartbeat (ADR-0032, ADR-0056). Gateway and controller share one runtime identity, which remains stable across replacement. Every terminal attachment and process termination checks that identity through `session-ownership.mjs` (ADR-0036). The controller's HTTP boundary is a set of capability route tables. Stateful launch, message-delivery, pane-observation, vault, and record mechanisms own their state behind narrow factories (ADR-0031). Browser code uses explicit ES modules. `shell.js` composes feature-owned ports, and the shell coordinator handles cross-feature navigation without flat dependency lists or service bags. One browser refresh coordinator serializes conditional Work reads from SSE, timers, mutations, and direct actions. It tracks gateway, controller, transport, and retry state separately. Provider is the fourth launch axis beside harness, model and effort: an optional registry field that a model option can override, resolved by `launch-environment.mjs` and stamped onto `resolvedLaunch.ref` when an attempt starts. It never enters the three-part `launchRef()` string. `harness-transcripts.mjs` is the single place that knows where each harness family writes a conversation and its subagents, and both liveness and cost read it. `conversation-cost.mjs` and `job-cost.mjs` price a conversation from its transcripts, preferring a harness's own ledger while that ledger is the last word, and `cost-service.mjs` serves `GET /api/cost/workers` from an index it refreshes behind the request, keyed by Goal file and by tmux session (ADR-0059). Rates come from a seeded table in `pricing-catalog.mjs` and from `~/.tangent/trees/pricing.md`, which overrides it; a model with no rate is reported unpriced rather than guessed (ADR-0057). Vault Markdown and workflow records stay readable while private module and loopback contracts may change atomically with their callers.

...
Recurring dispatch: a recurring job is a `recur-<slug>.md` vault file. Frontmatter contains `schedule`, `cwd`, optional `model`, and optional `paused`. The body is the worker prompt. A paused definition stays visible but is not due. `tangent threads recur due` scans the vault, runs every due definition, and records each fire in the sidecar. `--dry-run` reports what would run without a launch. `tangent threads recur run <slug>` runs one definition regardless of due state. The launchd timer template lives at `assets/com.tangent.threads-recur.plist`. Use a dedicated worktree for `cwd` when possible. A shared directory can misattribute the user's own sessions to the thread.

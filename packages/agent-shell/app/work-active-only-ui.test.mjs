...
/** Returns the Active-only control owned by the visible Work lens. */
function activeButton(document) {
  return document.querySelector("#work-lens-content [data-active-only]");
}
...
test("A keeps the Areas with a live brain or a running agent and drops the rest; A again shows everything", async () => {
  ...
  const button = activeButton(document);
  ...
  assert.equal(activeButton(document).getAttribute("aria-pressed"), "true");
  ...
  assert.equal(activeButton(document).getAttribute("aria-pressed"), "false");

  activeButton(document).click();
  await settle(window);

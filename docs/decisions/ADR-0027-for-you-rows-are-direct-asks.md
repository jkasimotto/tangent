...
- One ask datatype, one constructor, one renderer. `prototypes/agent-shell/public/ask-core.js` builds `{ area, subject, detail, question, actions, source }` and refuses anything without a question that ends in `?` and at least one answer action. Every source of rows passes through one of its builders, and `askRow` in `shell.js` draws all of them. A non-ask is unrepresentable.

 ...
 * most-recent activity. Pure, so usage-ui-data tests cover it directly.
 */
export function groupSessionsByProject(sessions: UsageSessionListItem[]): UsageProjectRailItem[] {
  ...
  for (const session of sessions) {
    const label = session.project || UNKNOWN_PROJECT;
  ...
  return [...groups.values()].sort((left, right) => (right.lastActivityAt || "").localeCompare(left.lastActivityAt || ""));
}

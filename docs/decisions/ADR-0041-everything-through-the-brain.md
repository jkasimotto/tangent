...
3. The brain marks Goals done. A worker sends one plain note to the organizer Area in its prompt. Tangent stores the Assignment handover and receipt. The brain reads the note, then marks the Goal done or appends a review step. `completionPolicy`, `designatedReview`, and the review-only closure rule are gone. Old queue records keep the fields and nothing reads them.
4. No automatic brain handover or rotation. The old generated-handover, pacing, 429 refusal, 90-minute reminder, and `wakeFromPaceText` remain gone. ADR-0055 later adds deliberate `tangent brain succeed`: it stages the next generation without authority and promotes it only after changed Area memory, a self-sent notice, and exact first-message transcript proof. `record.checkpoint` stays readable.

...
test("normal reading keys match exact Vim and Vimium commands", () => {
  ...
  assert.equal(matchDocumentReadingCommand(key("KeyG")), command.stageTop);
  assert.equal(matchDocumentReadingCommand(key("KeyG"), { pendingG: true }), command.top);
  ...
  assert.equal(matchDocumentReadingCommand(key("KeyN")), command.nextComment);
  assert.equal(matchDocumentReadingCommand(key("KeyN", { shiftKey: true })), command.previousComment);
...
test("Escape is staged and read-only readers keep navigation without comment creation", () => {
  ...
  assert.equal(matchDocumentReadingCommand(key("KeyN"), { commentCreation: false }), command.nextComment);
  assert.equal(matchDocumentReadingCommand(key("KeyN", { shiftKey: true }), { commentCreation: false }), command.previousComment);
  assert.equal(matchDocumentReadingCommand(key("KeyN"), { commentNavigation: false }), null);
...
test("wrong modifiers, IME input, and an earlier owner never reach reading mode", () => {
  ...
  assert.equal(matchDocumentReadingCommand(key("BracketLeft")), null);

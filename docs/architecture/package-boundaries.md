...
Vertical apps:
- @tangent/agent-shell owns the vault CLI (`tangent area`, `tangent goal`, `tangent vault commit`), the Job CLI (`tangent job`), the Agent CLI (`tangent agent` and `tangent send`), the Brain CLI (`tangent brain`, `tangent area create`), the server CLI (`tangent shell rebuild`), and `tangent study`, lazily loaded from `@tangent/agent-shell/cli`. Goal commands own intent; Job commands own execution; Agent commands own sessions; Brain commands own Area organization. Every command but `vault commit` and `study` is a thin HTTP client to Agent Shell.
- The Agent Shell app in `packages/agent-shell/app/` has one stable gateway and one supervised controller (ADR-0032). They share one runtime identity. The session ownership capability creates, attaches, and terminates only tmux sessions with that identity (ADR-0036). The gateway owns the public listener, terminal transport, static assets, SSE, and the atomic Work store. The controller observes exact Work sources and publishes bounded candidates. It also owns exact-Area Brains, Goal intent projection, `job.v1` history, Agent Attempts, the harness registry and Area policies, durable inboxes and receipt recovery, Requests, presentations, structured lifecycle events, invariant Problems, and detached audit exports (ADR-0054, ADR-0055, ADR-0056). No other vertical package imports these runtime record types.
- The Agent Shell controller also owns server-authoritative worker and brain state, bounded recovery ladders, and exact-Area repair crews. Repair records, leases, target fences, and command audits stay inside Agent Shell. Work and the agent CLI read the same projected state. See ADR-0047.
- The Agent Shell package owns the Area Excalidraw source shards and their composed browser world. It also owns the shared layout kernel, identity mapping, history, migration, transactions, recovery, and world HTTP routes. No other vertical package writes Area scenes (ADR-0049, ADR-0051, ADR-0052).
- The Agent Shell package owns Area resource catalogs and their private loopback APIs. Its CLI is the only supported non-browser management surface.
- Resource discovery uses public `@tangent/repo` Git helpers. Provider status uses injected named readers, and no provider integration exists by default.
...
- agent-shell must not import Eval, Usage, Rollup, or Search.

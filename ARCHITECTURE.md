...
Usage owns conversation telemetry: event schemas, datasets, SDKs, CLI, usage CLI, hook input normalization into Usage events, native transcript normalization, and native-log schema compatibility checks. Hooks owns provider config mechanics only. Native provider transcripts are high-signal but unstable telemetry inputs; Usage indexes them by default behind normalized Usage APIs and retains hooks as explicit legacy/debug capture. Daily and Eval may consume Usage data, but Usage must not learn about Daily, Eval, or Search.

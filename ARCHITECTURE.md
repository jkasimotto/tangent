...
Agent Shell is the daily work product. The local prototype in `prototypes/agent-shell/` connects vault outcomes, durable user notes, tmux sessions, and derived attention signals. Its default entry presents Work, Summary, Plan, Agent, and Next step. The previous tree-first interface remains at `/legacy` during migration.

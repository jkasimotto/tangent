    ...
    return `
      <section class="area-contents area-map-screen ${done ? "area-done" : ""}">
        ...
        <details class="area-more"><summary>More</summary>
          ...
          <details><summary>Area settings</summary><div class="area-settings-actions"><button class="quiet-button" type="button" data-launch-for="brain" data-brain-area="${escapeHtml(area.path)}">Set brain agent and effort</button><button class="quiet-button" type="button" data-new-area>Add nested Area</button>${area.path.split("/").length > 1 ? `<button class="quiet-button" type="button" data-rename-area>Rename or move</button>` : ""}${done ? `<button class="quiet-button" type="button" data-reopen-area="${escapeHtml(area.path)}">Reopen</button>` : `<button class="quiet-button" type="button" data-mark-area-done="${escapeHtml(area.path)}">Mark done</button>`}</div></details>
        </details>
      </section>`;
  }

...
test("the Work table carries one Cost cell per Goal row, keyed by the Goal file", async () => {
  ...
  const { document } = await bootWorkTable(fixture, {
    workerCost: {
      work: {
        ...
        "brain:otto/tangent": entry({ amount: 9, display: "$9" }),
      },
      sessions: {},
    },
  });
  ...
  // An Area brain has no row of its own, so its figure rides in the group header.
  assert.equal(document.querySelector('[data-worker-cost="brain:otto/tangent"]').textContent.trim(), "$9");

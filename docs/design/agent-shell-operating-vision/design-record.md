...
**D20. A skill is `<area>/skill-<slug>.md` (Julian).** An ordinary Document. `brainPrompt` gains an optional `## Skills` section with one line per skill, `- <title>: <abs path> (<Area>)`, at most 12, root to leaf. The section also names the bound repository's own project skills. The brain hands a skill to a worker with `--source <vault-file>` or by naming the path in the instruction.

...
Root CLI:
- Owns human command taxonomy (`setup`, `status`, `ui`, `usage`, `rollup`, `eval`, `doctor`) and may compose installed product commands and UI app descriptors through lazy imports.
...
- `@tangent/usage`, `@tangent/rollup`, and `@tangent/eval` must be publishable and installable independently.
...
- Standalone CLIs use `tangent-usage`, `tangent-rollup`, and `tangent-eval`; the root `tangent` package keeps short subcommands.

Hard rules:
- rollup and eval may depend on dependency-light Usage data packages, not the full Usage app or Usage UI packages.
- usage must not depend on rollup or eval.
...
- UI platform packages must not import Usage, Eval, or Rollup product packages.

...
test("Work Escape unwinds each state in the settled order", async () => {
  ...
  await settle(window);
  assert.equal(document.querySelector("[data-starred-only]").getAttribute("aria-pressed"), "true", "F shows only the starred Areas");
  document.querySelector("[data-work-object-actions]").click();
  await settle(window);
  document.querySelector("[data-modal-action='chooseAreas']").click();
  await settle(window);
  ...
  assert.equal(document.querySelector("[data-starred-only]").getAttribute("aria-pressed"), "true", "only starred remains until the next Escape");
  press(window, "Escape");
  await settle(window);
  assert.equal(document.querySelector("[data-starred-only]").getAttribute("aria-pressed"), "false", "4: only starred");
  assert.ok(document.querySelector(".work-star.starred"), "the stars remain until the next Escape");
  press(window, "Escape");
  await settle(window);
  assert.equal(document.querySelector(".work-star.starred"), null, "5: the stars");

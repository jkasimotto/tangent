...
There is no `tangent mark` CLI command; it was removed 2026-08-15 (ADR-0020). `createMarkRecord` still fills in id/at/status/anchor/repo defaults for any caller writing a mark, and `server/marks-routes.ts` reads and updates marks directly for the Eval UI's marks inbox (list/show/update, the triage-to-fix workflow).

`marks/to-eval.ts`'s `runToEval({ markId, ... })` promotes a mark into a runnable eval scaffold (no CLI front-end since the `tangent mark` command was removed 2026-08-15, ADR-0020). It derives the eval slug from `--name` or the mark's own id, selects the task prompt (the user message at or nearest before the mark's anchor, read via `@tangent/usage-index-sqlite`'s `readConversationsUserMessages`, falling back to a stub drafted from the mark's own `observed`/`expected`/`hypothesis` text when the session is not indexed), and writes `evals/<slug>/{eval.json,prompts/task.md,README.md}` into the mark's repo. The generated `eval.json` carries one case with `baseline`/`fixed` variants in `snapshot` context mode, pointing at context refs the user captures with `tangent eval context capture` (the exact commands are in the generated README, since `eval.json` has no comment syntax), and an `evaluator` block with an explicit judge model and binary criteria drafted one-per-sentence/clause from the mark's `expected` text, per ADR-0013. On success the mark is updated to `status: "eval-created"` with `links.eval` set.

`marks/scan.ts` (with `scan-candidates.ts` and `scan-runner.ts`) implements `scanForSuggestedMarks(...)`, the phase-3 sweep (no CLI front-end since the `tangent mark` command was removed 2026-08-15, ADR-0020): it seeds candidate conversations from the deterministic insight generators plus user-message activity, sends each capped candidate to one cheap-model call through the claude CLI (model is a required option; the CLI flag defaults to haiku explicitly), and writes the returned incidents as `status: "suggested"` marks for inbox curation. Conversations already carrying a non-dismissed mark are skipped, malformed model responses are counted and skipped, `--dry-run` writes nothing, and nothing in the sweep creates evals or edits context files.

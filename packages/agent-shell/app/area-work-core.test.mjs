...
const goal = (file, title, area, dependsOn = []) => ({
  file, title, area, status: "open", dependsOn, requiredBy: [], changedAt: 1,
});

test("a reduced Work graph keeps a match's prerequisite context", () => {
  const first = goal("neara/pgande/benchmarking/goal-a.md", "PLDB mismatch", "neara/pgande/benchmarking");
  const second = goal("neara/pgande/benchmarking/goal-b.md", "Force difference", "neara/pgande/benchmarking");
  const blocked = goal("neara/pgande/benchmarking/goal-c.md", "Benchmark poles", "neara/pgande/benchmarking", [
    { file: first.file, title: first.title, status: "open" }, { file: second.file, title: second.title, status: "open" },
  ]);
  const result = core.project({ scope: "neara/pgande/benchmarking", goals: [first, second, blocked], areaPaths: [], filters: { query: "benchmark poles" } });
...
test("the Work graph offers no person or assignee filter", () => {
  const only = goal("otto/team/goal-a.md", "Only", "otto/team");
  const result = core.project({ scope: "otto/team", goals: [only], areaPaths: [], filters: { person: "otto/team::julian" } });
  assert.equal(result.reduced, false);
  assert.deepEqual(result.frontier.map((item) => item.goal.title), ["Only"]);
  assert.equal(core.matchesPerson, undefined);
});

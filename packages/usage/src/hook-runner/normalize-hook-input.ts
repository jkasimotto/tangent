...
import type {
  ...
  UsageProvider,
  TrackingSource
} from "../core/schema/usage-jsonl-v1.js";
...
export function normalizeHookInput(input: Record<string, unknown>, context: NormalizeContext): UsageJsonlLineV1[] {
  ...
  const base = {
    ...
    capture: {
      ...
      scope: context.scope,
      usage_version: context.usageVersion,
  ...
  const line = (kind: UsageEventKind, data: unknown, extra: Partial<UsageJsonlLineV1> = {}): UsageJsonlLineV1 => ({
    ...base,
    event_id: eventId(),
    kind,
    data,
  ...
  switch (hookName) {
    ...
    case "PostToolUse":
      return [
        ...
        ...usageEventsFromToolResponse(input, line)
      ];
    case "PostToolUseFailure":
      return [
        line(
          ...
          {
            ...toolData(input, context.redaction, "result"),
            error: redactUnknown(input.error || input.tool_error || input.tool_response, context.redaction),
            status: "error"
          },
          { turn: turn(input), actor: { role: "tool", model: asString(input.model) }, links: { tool_call_id: asString(input.tool_use_id) } }
        )
    ...
    case "PermissionDenied":
      return [
        line(
          ...
          { turn: turn(input), actor: { role: "hook", model: asString(input.model) } }
        )
      ];
    case "PostToolBatch":
      return [
        line(
          "capability.notice",
          {
            ...
            tools: redactUnknown(input.tools || input.tool_uses || input.batch, context.redaction)
          },
          { turn: turn(input), actor: actor(input).actor, availability: { confidence: "partial", notes: ["Captured from provider batch-level hook."] } }
        )
      ];
      ...
      return [
        line(
          "message.system",
          {
            ...
            files: redactUnknown(input.files || input.instructions || input.loaded_instructions, context.redaction)
          },
          { turn: turn(input), actor: { role: "system", model: asString(input.model) }, availability: { confidence: "partial" } }
        )
      ];
    case "Notification":
      return [
        line(
          ...
          { turn: turn(input), actor: actor(input).actor, availability: { confidence: "partial" } }
        )
      ];
    case "PreCompact":
...
function actor(input: Record<string, unknown>): Partial<UsageJsonlLineV1> {
  return { actor: { role: "hook", model: asString(input.model) } };
}

function usageEventsFromToolResponse(
  input: Record<string, unknown>,
  line: (kind: UsageEventKind, data: unknown, extra?: Partial<UsageJsonlLineV1>) => UsageJsonlLineV1
): UsageJsonlLineV1[] {
  const usage = extractUsage(input.tool_response);
  if (!usage) return [];
  return [
    line(
      "token.usage",
      ...
      {
        turn: turn(input),
        actor: { role: "tool", model: asString(input.model) },
        links: { tool_call_id: asString(input.tool_use_id) },
        availability: { confidence: "partial", notes: ["Provider tool response included usage fields."] }
      }
    )
  ];
}
...
function extractUsage(value: unknown): unknown {
  ...
  if (record.totalTokens || record.total_tokens || record.input_tokens || record.output_tokens) {
    return {

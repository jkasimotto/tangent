...
- `tangent goal handover --continue <facts...> [--session <name>] [--server]`: run by a worker whose carried context is filling, at a natural pause (ADR-0028). Hands the same step or Goal to a fresh copy of itself instead of advancing the pipeline: posts `POST /api/goals/handover { session, text, continue: true }`. The server writes the facts as a continuation entry, spawns a fresh session primed with the rebuilt step prompt plus every continuation's facts, and kills the old session only once the new one confirms it received the whole prompt. Prints `handed over; a fresh copy continues this step: <session>`. Also works from a solo (non-pipeline) Goal session. A session whose step already moved to a fresh one is refused 409, naming the live session; a session that is neither a running step nor a Goal session is refused 404.

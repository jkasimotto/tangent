...
- Vault mutations: `POST /api/goals/create` (accepts `own: <session>`), `POST /api/goals/depend` and `POST /api/goals/undepend` (`{ slug, on: [prerequisiteSlug] }`), `POST /api/idea/new`, `POST /api/goals/edit`, `POST /api/goals/own` and `POST /api/goals/release` (`{ session, slugs }`), `POST /api/agents/send`.

...
- The shell's launcher starts outcome sessions as plain tmux shells in the node's repo, bound with `@tangent_node` and `@tangent_outcome`, with the outcome flipped to `active`. It does not launch an agent or type outcome context into the shell. You do not need to replicate that; when the user asks you for a session on a specific outcome, you may create it the same way (set both options, flip the status, commit).

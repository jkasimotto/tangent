...
- A stored index whose meta version differs from `searchIndexVersion` triggers an automatic full rebuild (`index-version-changed`), so extractor fixes reach existing indexes on the next `tangent search index` run without `--force`.

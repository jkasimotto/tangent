...
- The Agent Shell app in `packages/agent-shell/app/` has one stable gateway and one supervised controller (ADR-0032). The gateway owns the public listener, terminal transport, static assets, and SSE. The controller owns Goals, sessions, pipelines (ADR-0023), Area brains (ADR-0024), and the desk's `For you` list. Capability route tables own controller HTTP shapes. Launch catalog, message delivery, pane observation, vault mutation, and persisted record modules own their state behind narrow factories (ADR-0031). Browser factories receive feature-owned ports rather than flat dependency lists or generic service bags. `public/shell.js` is the browser composition root; `shell-coordinator.js` owns cross-feature navigation.

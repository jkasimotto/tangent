...
import { managedProcessSession, parseProgramManifest, programsSnapshot, saveLocalProgram } from "./programs.mjs";
...
test("a manifest that still declares triggers is refused with the process-note hint", () => {
  assert.throws(() => parseProgramManifest('{"triggers":{"rebase":{"every":"1d","probe":"./probe","instructions":"RUN.md"}}}', "neara/pgande/.processes.json"), /triggers retired; write a process-<slug>.md note/);
});

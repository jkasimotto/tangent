...
- `src/cli/commands/{area,goal,handover,brain,idea,document,agent}.ts`: thin HTTP clients to the Agent Shell server. Both worker handover commands use one strict report parser and one HTTP route. Brains can give an explicit caller session outside tmux. The server stores Goal queues and Request records. Goal mutations require the current live brain for the exact target Area.

...
- `createEvalUiApp(...)` builds the Eval app registration. It mounts `/api/eval/*` routes and assets under `/apps/eval/`. `startEvalUiServer` consumes it.
...
- There is no `tangent mark` CLI command; it was removed 2026-08-15 (ADR-0020). The marks store, scan, and to-eval modules (`packages/eval/src/marks/`) stay: `packages/eval/src/server/marks-routes.ts` reads and updates marks directly for the Eval UI's marks inbox, independent of any CLI.

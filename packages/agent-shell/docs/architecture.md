...
- `brain-native-turns.mjs`: native transcript resolution, explicit memory cues, and changed-file polling through the Usage CLI.
...
Brain attempts store native conversation locators. Claude gets an identifier before launch. Codex uses the attempt folder and start time. The transcript monitor uses the native message identifier as the Journal idempotency key. Agent Shell calls `usage native messages` for normalization and does not import Usage (ADR-0044).

...
- `worker-questions.mjs`: stable question identity, exact answer and acknowledgement transitions, replacement transfer, and prompt projection;
...
A `question-needed` report contains a stable question ID and state. Only the exact current Area brain can answer it through a send to an attempt in that assignment. The queue commit changes a waiting assignment to running before the command returns. `tangent send brain --question` then receives the answer as command output and acknowledges the exact attempt. Replacement promotion transfers an open or unacknowledged question. Rebuilt prompts contain open questions and answers. This control path does not inspect or write a harness composer (ADR-0050).

...
The package is `src/cli/` and nothing else. The root `tangent` CLI lazily loads `@tangent/agent-shell/cli` for the `area`, `brain`, `goal`, `idea`, `document`, `vault`, `agent`, and `shell` nouns, the same way it loads `usage`, `eval`, `rollup`, `search`, and `threads`.

- `src/cli/spec.ts`: the help specs (`areaCommandSpec`, `brainCommandSpec`, `goalCommandSpec`, `ideaCommandSpec`, `documentCommandSpec`, `agentCommandSpec`, `shellCommandSpec`, `vaultCommandSpec`).
...
- `src/cli/commands/shell.ts`: `tangent shell rebuild`. It posts the server's own rebuild endpoint and then polls `GET /api/sessions` until the boot id changes, so a brain that writes a Try it note knows the running server already answers with the new code (ADR-0025).

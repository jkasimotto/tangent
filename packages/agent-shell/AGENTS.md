...
- Goal execution is owned by one server-side `area-goal-queue.v2` record. Each assignment records its organizing brain's durable Area path. Workers use only `tangent send <brain-area> "<plain note>"`; a note never mutates workflow state.
...
- Keep the package free of browser code and of Eval, Usage, Rollup, and Search imports.

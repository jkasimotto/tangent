...
Status: implemented on 2026-08-28 (commits 50080a1 and 3f49a81). Every not-done Area has a row, on Julian's word on 2026-08-28: "I can't see areas that don't have goals". Each top-level Area is one Work header, with `0 open` when nothing is under it. Each not-done sub-Area under an open header is one sub-header row, with or without Goals or a brain. A quiet sub-Area prints muted so the eye skips it, and keeps its fold, its brain button, and its menu. A done Area, or one under a done Area, keeps its row only while it has open Goals. `{`, `}`, `j`, and `k` walk the new rows the same way. The depth-2 root rule and the live-brain-owns-the-subtree rule in `deskPanels` are gone. Focus roots still own their subtree. Two deviations from the text below, both accepted at review: a live brain at depth 3 or deeper is now work inside its top-level group, not a peer panel (deskPanels changed). A sub-header counts only its own brain's questions. The top-level header keeps the roll-up. A folded sub-header hides its whole subtree and counts it, on Julian's word on 2026-08-28: "When I collapsed otto/dnd it did not collapse otto/dnd/players".

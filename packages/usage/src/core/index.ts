...
export async function openUsage(options: OpenUsageOptions = {}): Promise<UsageClient> {
  if (options.index !== false) {
    try {
      const sqlite = await import("../sqlite/index.js");
      return await sqlite.openUsageFromSqlite(options);
    } catch (error) {
      if (options.index !== "auto" && options.index !== undefined) throw error;
      if (!isOptionalSqliteFailure(error)) throw error;
    }
  }

  const providers = await import("../providers/index.js");
  const loaded = await providers.loadProviderEvents(options);

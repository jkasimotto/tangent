...
- `tangent shell rebuild [--server <url>] [--timeout <seconds>]`: prints the commits between the running server's deployed revision and repository `HEAD`, posts `POST /api/shell/rebuild`, then polls `GET /api/sessions` every 500 ms until `boot` changes, so the command returns only when the new code answers. Default timeout 240 s; the failure names the rebuild log. The brain runs it before it writes a Test line. Uncommitted filesystem edits do not advertise an update.
...
- Read: `GET /api/tree`, `GET /api/areas/show?area=<path>`, `GET /api/goals[?area=<path>]`, `GET /api/goals/show?slug=<slug>`, `GET /api/ideas[?area=<path>]`, `GET /api/document/comments?file=<path>`, `GET /api/sessions`. The sessions snapshot includes `deployedCommit`, `currentCommit`, and `pendingCommits`; the blue update indicator appears only when the pending commit list is non-empty.

...
- If `role` is `worker`, read the Goal file, done condition, current assignment instruction, prior handovers, reports, and rebuilt `prompt`. Continue only when `current` is true. Finish through the handover command required by that prompt.
- If `promptError` is present, continue from the durable fields and use `tangent handover --help` for the report syntax. Do not treat a prompt rebuild failure as a lost assignment.

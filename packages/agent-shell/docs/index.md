...
The Agent Shell gateway in `packages/agent-shell/app/gateway.mjs` owns port 4321 and durable terminal transport. It supervises the controller in `server.mjs`, which owns Goals, human assignees, pipelines, and session projection. Human assignees supply labels and filters only. See ADR-0032.

...
test("reports SDK progress while indexing", async () => {
  ...
  const indexed = await indexRepo({ repo, force: true, onProgress: (event) => events.push(event) });
...
test("reports progress for up-to-date indexes", async () => {
  ...
  const indexed = await indexRepo({ repo, onProgress: (event) => events.push(event) });
...
test("reports slow operation warnings through SDK progress", async () => {
  ...
  await indexRepo({ repo, force: true, slowOperationMs: 0, onProgress: (event) => events.push(event) });

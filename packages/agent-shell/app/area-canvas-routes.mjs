...
export function createAreaCanvasRoutes({ repository, proposals = null, view = null, areaExists = async () => true }) {
  async function handle(request, response, url) {
    ...
    if (request.method === "GET") {
      ...
      const value = { area, file: result.file, exists: result.exists, hash: result.hash, canvas: result.canvas, warnings: result.warnings, proposals: proposals ? await proposals.list(area, { openOnly: true }) : [], view: view ? await view(area) : null };

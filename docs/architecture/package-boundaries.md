...
- The Agent Shell app in `packages/agent-shell/app/` serves the UI and owns Goals, sessions, pipelines (ADR-0023), Area brains (ADR-0024), and the desk's `For you` list, which under a live brain comes only from that brain's plan (ADR-0025). Capability route tables own HTTP shapes. The composed server operations own product behavior. A repository module owns vault writes and commits. The server projects desk grouping and attention for the browser. Browser product modules own state, API access, DOM lookup, Markdown structure, goal narrative, finder projection, Area maps, comments, text formatting, Work desk/search rendering, Area and Program surfaces, Goal/launch/harness editing, agent/decision views, Document reading and navigation, terminal lifecycle, and event binding. Stateful boundaries are dependency-injected factories. `public/shell.js` is their composition root and owns active-screen selection plus refresh and paint coordination.

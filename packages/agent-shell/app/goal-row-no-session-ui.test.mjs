...
test("the Work row route of a Goal whose session is gone asks the brain, it never starts an agent itself", async () => {
  ...
  const title = document.querySelector(`[data-goal-anchor='${goal.file}'] [data-work-row-title]`);
  assert.equal(title.dataset.openGoalRun, goal.file, "the row keeps the run route");
  title.click();
  await settle(window, 3);

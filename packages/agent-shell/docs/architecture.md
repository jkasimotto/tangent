...
The browser Map itself is TypeScript under `app/map/`, built by `app/build-browser.mjs` into the one `agent-shell-map.js` bundle. It is held to a lint-enforced bar (ADR-0059, design in `docs/design/area-map-rebuild/code.md`): every number carries a unit or semantic brand from `app/map/units/`; the kernel under `app/public/` is reached only through `app/map/kernel/kernel-boundary.ts`; `app/map/input/press-meaning.ts` alone decides what a pointer press means and `app/map/input/excalidraw-subordination.ts` alone tells Excalidraw what it may hold selected before its first move frame; every surface is declared once in `app/map/surfaces/surface-registry.ts` and rendered through `app/map/ui/Surface.tsx`, which owns dialogs, backdrops and focus; every layout number is named once in `app/map/layout/layout-tokens.ts`; every sentence a person reads lives in `app/map/copy/`. The lints in `scripts/lint/` run on staged files at commit time and on the tree with `npm run lint`, with jscpd blocking duplicated code in that scope. Every value the server mints and later guards is registered beside its guard in `app/public/area-map-wire-values.js` and proved by one property test.

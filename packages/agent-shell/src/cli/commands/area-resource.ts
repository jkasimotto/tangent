...
import { booleanArg, requiredString, stringArg, stringsArg, type Args } from "@tangent/core/cli";
import { randomUUID } from "node:crypto";

import { postJson, requireArea, resolveServerUrl, vaultFetch } from "../client.js";
import { areaCommandSpec } from "../spec.js";

// `tangent area resource ...`: the thin HTTP client for the Map resource
// catalog (ADR-0049 Blocks, docs/design/map-entities). area.ts dispatches
// here and reuses the shared target text for `tangent area show`.

export type MapResourceLocator = { owner: string; id: string };
export type MapResourceTarget =
...
export function resourceFailureEnvelope(error: unknown, args: Args): Record<string, unknown> {
...
export async function resourceCommand(args: Args): Promise<void> {
...
export function printJson(value: unknown): void { console.log(JSON.stringify(value, null, 2)); }
...
export function resourceTargetText(target: MapResourceTarget | null | undefined): string {
